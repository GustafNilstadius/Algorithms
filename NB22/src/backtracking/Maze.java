...
public class Maze {
	...
	public Maze(){
		try{
			BufferedReader in = new BufferedReader(new FileReader("Labyrint_2.txt"));
	...
	private boolean solve(Position p){
        ...
        if (p.equals(goal)){
            ...
            if (mazeMatrix[p.row-1][p.column].equals(Cell.OPEN) && !mazeMatrix[p.row-1][p.column].equals(Cell.VISITED)) {
                if(solve(new Position(p.row-1, p.column))){
                    return true;
                }
            }
            if (mazeMatrix[p.row][p.column+1].equals(Cell.OPEN) && !mazeMatrix[p.row][p.column+1].equals(Cell.VISITED)){
